  ...
  return (
    <main id="main-content" className="relative bg-[var(--background)] text-black min-h-screen selection:bg-[#7B2CBF]/20 selection:text-black">
      ...
      <div className="pt-40 pb-32 px-6 container mx-auto relative overflow-hidden bg-[var(--background)]">
        ...
        <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-instrument text-black tracking-tight mb-6 md:mb-8 leading-[1.05]">
          The person <span className="font-instrument text-[#7B2CBF] italic">building</span> <br />Aifloxium.
        </h1>
      ...
      <section className="py-24 md:py-32 bg-[var(--background)] text-black relative z-20 border-y border-black/5">
        <div className="container mx-auto px-6 max-w-5xl">
          <div className="text-center mb-20">
            <h2 className="text-xl md:text-4xl font-instrument tracking-tight mb-4">How the work evolved</h2>
            <p className="text-black font-inter font-light">My progression from workflow scripting to autonomous agentic architectures.</p>
          </div>
          
          <div className="space-y-16 pl-4 md:pl-0 border-l border-black/10 md:border-l-0">
            {[
              { year: "Phase 1", title: "Workflow Automation Foundations", desc: "Engineered high-volume data integration pipelines and workflow scripts, helping businesses automate repetitive operations and manual data tasks.", icon: <Cpu className="w-5 h-5 text-[#7B2CBF]" /> },
              { year: "Phase 2", title: "Shift to Agentic AI & Vibe Coding", desc: "Adopted tools like Antigravity, Claude Code, and OpenAI Codex to build agentic operating systems, custom vibe-coded dashboards, and dynamic multi-agent tools.", icon: <Database className="w-5 h-5 text-[#7B2CBF]" /> },
              { year: "Phase 3", title: "Applied AI Systems & Voice Agents", desc: "Applying intelligent AI reasoning layers directly to client databases, running conversational Voice AI agents (<500ms latency), and delivering white-label AI backends for global clients.", icon: <TrendingUp className="w-5 h-5 text-[#7B2CBF]" /> }
            ].map((item, i) => (
              <div key={i} className="relative flex flex-col md:flex-row gap-8 md:items-center">
                  <div className="hidden md:flex flex-col items-center absolute left-1/2 -translate-x-1/2 w-full h-full">
                    ...
                    <div className="absolute top-0 w-12 h-12 bg-[var(--muted)] border border-black/10 text-black rounded-full flex items-center justify-center shadow-lg -translate-y-1/2">
                    {item.icon}
                  </div>
                </div>
                ...
                <div className={`md:w-1/2 ${i % 2 === 0 ? 'md:pr-16 md:text-right' : 'md:pl-16 md:ml-auto'}`}>
                  ...
                  <h3 className="text-2xl font-semibold font-inter mb-4 text-black">{item.title}</h3>
                  <p className="text-black font-inter font-light leading-relaxed bg-[var(--muted)] p-6 rounded-[1.5rem] border border-black/5 shadow-sm">{item.desc}</p>
                </div>
      ...
      {/* Accordion Section */}
      <section className="py-24 md:py-32 bg-[var(--background)] relative overflow-hidden border-t border-black/5">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center mb-16">
            <h2 className="text-xl md:text-4xl font-instrument text-black tracking-tight mb-4">Working style</h2>
            <p className="text-black font-inter font-light">How I approach technical execution, code quality, and system delivery.</p>
          </div>
          <div className="dark">
            <Accordion05 />
          </div>

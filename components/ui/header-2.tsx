...
export function Header() {
	...
	const scrolled = useScroll({ threshold: 10 }).scrolled;
	...
	return (
		<header
			className={cn(
				'sticky top-0 z-50 mx-auto w-full max-w-5xl border-b border-transparent md:rounded-xl md:border md:transition-all md:duration-500 md:ease-out',
				{
					'bg-background/95 supports-[backdrop-filter]:bg-background/50 border-border/50 backdrop-blur-xl md:top-4 md:max-w-4xl md:shadow-2xl md:shadow-black/20':
						scrolled && !open,
		...
		>
			<nav
				className={cn(
					'flex h-16 w-full items-center justify-between px-6 md:h-14 md:transition-all md:duration-500 md:ease-out',
					{
						'md:px-4': scrolled,
					},
				)}
			>
				...
				<div className="hidden items-center gap-1 md:flex">
					{links.map((link, i) => (
						<motion.a 
							...
							className={buttonVariants({ variant: 'ghost', className: 'px-4 hover:bg-primary/10 transition-colors' })} 
							href={link.href}
						>
							{link.label}
						</motion.a>
				...
				>
					<MenuToggleIcon open={open} className="size-6" duration={400} />
				</Button>
			</nav>
			...
			<AnimatePresence>
				{open && (
					<motion.div
						...
						className="bg-background/98 fixed top-16 right-0 left-0 z-50 flex flex-col overflow-hidden border-t backdrop-blur-2xl md:hidden"
					>
						<motion.div
							...
							className="flex h-full w-full flex-col justify-between gap-y-6 p-8"
						>
								...
								{links.map((link, i) => (
									<motion.a
										...
										className={buttonVariants({
											variant: 'ghost',
											className: 'justify-start text-xl py-6 hover:bg-primary/10',
										})}
										href={link.href}
							...
							>
								...
								<Button size="lg" className="w-full text-lg py-7 rounded-full shadow-xl shadow-primary/20">
									Get Started
								</Button>
							</motion.div>
						</motion.div>
